...
class SpeechCommands(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Read waveform
        filename = self.file_list[index]
        raw_audio, _ = torchaudio.load(filename)

        # Apply augmentation
        x = self.transf(raw_audio).squeeze(dim=0)
        ...
        return x, y, raw_audio.squeeze(dim=0)

    def generateBatch(self, batch):

        xs, ys, raw_audio = zip(*batch)
        xlens = torch.tensor([x.shape[0] for x in xs])
        xs = torch.nn.utils.rnn.pad_sequence(xs, batch_first=True)
        ys = torch.LongTensor(ys)
        raw_audio = torch.nn.utils.rnn.pad_sequence(raw_audio, batch_first=True)
        return xs, raw_audio, ys
