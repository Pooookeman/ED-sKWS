...
def ES_alpha_acc(output, y, alpha=0.9):
    ...
    acc = np.mean((y == pred).detach().cpu().numpy()) 
    return acc, stop_timestep.float().mean()
...
class Experiment:
    ...
    def init_exp_folders(self):
        ...
        # Generate a path for new model from chosen config
            ...
            exp_folder = os.path.join("exp_100words", self.exp_name, outname.replace(".", "_"))
    ...
    def init_dataset(self):
        ...
        self.train_loader = load_100words(
        ...
        self.valid_loader = load_100words(
            dataset_name=self.dataset_name,
            data_folder=self.data_folder,
            split="valid",
            batch_size=self.batch_size,
        ...
        self.test_loader = load_100words(
            dataset_name=self.dataset_name,
            data_folder=self.data_folder,
            split="test",
            batch_size=self.batch_size,
    ...
    def train_one_epoch(self, e):
        ...
        for step, (x, frame_info, y, raw_audio) in enumerate(self.train_loader):
            ...
            output, firing_rates, _ = self.net(x)
            ...
            ave_end_time += frame_info[:, 1].float().mean()
        ...
        # End time statistic
        logging.info(f"Average end time ={ave_end_time/step}")
    ...
    def valid_one_epoch(self, e, best_epoch, best_acc):
        ...
        with torch.no_grad():
            ...
            for step, (x, frame_info, y) in enumerate(self.valid_loader):
    ...
    def test_one_epoch(self, test_loader):
        ...
        with torch.no_grad():
            ...
            for step, (x, frame_info, y, raw_audio) in enumerate(test_loader):
