...
class SNN(nn.Module):
    ...
    def forward(self, x):
        ...
        temporal_firing_rate = torch.cat(all_spikes, dim=2).mean(dim=(2))
        return x, firing_rates, temporal_firing_rate
...
class ReadoutLayer(nn.Module):
    ...
    def _readout_cell(self, Wx):
        ...
        out_list = torch.zeros(Wx.shape[1], Wx.shape[0], Wx.shape[2]).to(device)
